...
class PricePointTest {
  ...
  void constructor_validArgs_createsObject() {
    PricePoint p = new PricePoint(DATE, 171.06, 172.41, 168.88, 170.62, 169.50, 164_420_000L);
  ...
  void constructor_zeroVolume_isAllowed() {
    ...
    assertDoesNotThrow(() ->
        new PricePoint(DATE, 100.0, 110.0, 90.0, 105.0, 99.0, 0L));
  }
  ...
  void constructor_highEqualsLow_isAllowed() {
    ...
    assertDoesNotThrow(() ->
        new PricePoint(DATE, 100.0, 100.0, 100.0, 100.0, 100.0, 1000L));
  }
  ...
  void constructor_nullDate_throwsIllegalArgument() {
    assertThrows(IllegalArgumentException.class, () ->
        new PricePoint(null, 100.0, 110.0, 90.0, 105.0, 99.0, 1000L));
  }

  @Test
  void constructor_zeroOpen_throwsIllegalArgument() {
    assertThrows(IllegalArgumentException.class, () ->
        new PricePoint(DATE, 0.0, 110.0, 90.0, 105.0, 99.0, 1000L));
  }

  @Test
  void constructor_negativeHigh_throwsIllegalArgument() {
    assertThrows(IllegalArgumentException.class, () ->
        new PricePoint(DATE, 100.0, -1.0, 90.0, 105.0, 99.0, 1000L));
  }

  @Test
  void constructor_highLessThanLow_throwsIllegalArgument() {
    assertThrows(IllegalArgumentException.class, () ->
        new PricePoint(DATE, 100.0, 89.0, 90.0, 95.0, 99.0, 1000L));
  }

  @Test
  void constructor_negativeVolume_throwsIllegalArgument() {
    assertThrows(IllegalArgumentException.class, () ->
        new PricePoint(DATE, 100.0, 110.0, 90.0, 105.0, 99.0, -1L));
  }
  ...
  void getDailyRange_returnsHighMinusLow() {
    PricePoint p = new PricePoint(DATE, 171.06, 172.41, 168.88, 170.62, 169.50, 1000L);
  ...
  void getOpenToCloseChange_positiveMove_returnsCorrectPercent() {
    ...
    PricePoint p = new PricePoint(DATE, 100.0, 115.0, 95.0, 110.0, 99.0, 1000L);
    assertEquals(10.0, p.getPercentChange(p.getOpen()), 1e-9);
  }

  @Test
  void getOpenToCloseChange_negativeMove_returnsNegativePercent() {
    // open=100, close=90 → -10%
    PricePoint p = new PricePoint(DATE, 100.0, 105.0, 85.0, 90.0, 99.0, 1000L);
    assertEquals(-10.0, p.getPercentChange(p.getOpen()), 1e-9);
  }

  @Test
  void getOpenToCloseChange_flatDay_returnsZero() {
    PricePoint p = new PricePoint(DATE, 100.0, 100.0, 100.0, 100.0, 100.0, 1000L);
    assertEquals(0.0, p.getPercentChange(p.getOpen()), 1e-9);
  }

  // ── getPercentChange ──────────────────────────────────────────────────────

  @Test
  void getPercentChange_previousClose_returnsCorrectPercent() {
    // close=170.62, previousClose=168.00 → (170.62-168.00)/168.00 * 100
    PricePoint p = new PricePoint(DATE, 171.06, 172.41, 168.88, 170.62, 168.00, 1000L);
    assertEquals((170.62 - 168.00) / 168.00 * 100, p.getPercentChange(168.00), 1e-9);
  }

  @Test
  void getPercentChange_zeroPreviousClose_throwsIllegalArgument() {
    PricePoint p = new PricePoint(DATE, 171.06, 172.41, 168.88, 170.62, 169.50, 1000L);
    assertThrows(IllegalArgumentException.class, () -> p.getPercentChange(0.0));
  }
